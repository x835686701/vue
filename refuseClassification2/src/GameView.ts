...
class GameView extends ui.gameUI {
    ...
    private score: number = 0;      //总积分
    private time:number = 60;       //倒计时
    private currScore:number = 0;   //当前关卡得分，每关重新记录
    private targetScore:number = 25;  //目标分数，每关都会增加
    private ape: Garbage[] = [];
    private i:number = 0;
    public  currnode : any;
    private scoreCallBackHd:Laya.Handler = Laya.Handler.create(this,this.setScore,null,false);
    private site:any = [];
    private isfrist = true;
    private GarId:any;

    public position = [{x:211,y:375},{x:480,y:369},{x:215,y:598},{x:481,y:595},{x:215,y:807},{x:476,y:805}];
    constructor(_level:number,_score:number,_time:number,_currScore:number,_targetScore:number) {
        super();
        this.level = _level;
        this.score = _score;
        this.time = _time;
        this.currScore = _currScore;
        ...
        // this.GameBg.skin = "war/bg/neijiangbg"+num+".png";
          ...
          this.on(Laya.Event.MOUSE_DOWN, this, this.mouse_down);     //绑定鼠标按下事件
    ...
    //垃圾拆分方法
    public GarSplit(ape:any,id:any,sroreNo:any):void{
   ...
   public createGar2():void{
    ...
    private Garbageaddlevel():void{
        for(var i in this.ape){
           this.ape[i].level += 1;
        }
    }

    private removeArray(id):void{
    ...
    //加分方法
    private addscore(Ttype:any):void{
        var Trash;
        if(Ttype.type == 1){
            Trash = this.Trash1;
        ...
        setTimeout(()=>{Trash.skin = "war/Trash"+Ttype.type+".png";},300);
    ...
    gameOver():void{
        ...
        this.removeSelf();
        var gameover:GameOver = new GameOver(this.score,this.level);
        Laya.stage.addChild(gameover);
    }

 
  public  mouse_down():void{
     for(var i in this.ape){
    ...
    //错误提示
    public showMask(type:number,skin:string,name:any){
